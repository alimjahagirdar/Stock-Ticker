...
def welcome():
    if person["is_logged_in"] == True:
        ...
        nif100 = nse100.fast_info["lastPrice"]
        ...
        nifbank = niftybank.fast_info["lastPrice"]
...
def search():
    ...
    PRICE = tickers.tickers[search].fast_info["lastPrice"]
    ...
    fig.add_trace(go.Line(x = hist.index,y = hist['Close'],name='Price'),secondary_y=False)
    ...
    open = tickers.tickers[search].fast_info["open"]
    close = tickers.tickers[search].fast_info["previousClose"]
    thigh = tickers.tickers[search].fast_info["dayHigh"]
    tlow = tickers.tickers[search].fast_info["dayLow"]
    flow = tickers.tickers[search].fast_info["yearLow"]
    fhigh = tickers.tickers[search].fast_info["yearHigh"]
    mc = tickers.tickers[search].fast_info["marketCap"]
    a = numerize.numerize(mc)
    #roe = tickers.tickers[search].info["returnOnEquity"]
    pe = tickers.tickers[search].info["trailingPE"]
    macap = "{0:,.2f}".format(pe)
    eps = tickers.tickers[search].info["epsTrailingTwelveMonths"]
    ...
    #de=tickers.tickers[search].info["debtToEquity"]
    ...
    In = In.iloc[1:, :]
    inc = In.to_html(header="true", table_id="table",border = '0',index='false',col_space='10')
    ...
    balance = Bal.to_html(header="true", table_id="table",border ='0',index='false',col_space=25)
    ...
    cas= cash.iloc[1:, :]
    cashflow=cas.to_html(header="true", table_id="table",border = '0',col_space=25)
    ...
    if person["is_logged_in"] == True:
        ...
        return render_template('stock.html',display=True,price=PRICEa,Date = date,stock_name=Stock_name,ticker=search,change=growtha,graph=graph,company=comapny,name=name,
                             Open=open,Close=close,thigh=thigh,tlow=tlow,fthigh=fhigh,ftlow=flow,mkc=a,PE=macap,EP=eps,PB=b,
        ...
        return render_template('stock.html',display=False,price=PRICEa,Date = date,stock_name=Stock_name,ticker=search,change=growtha,graph=graph,company=comapny,name=name,
                             Open=open,Close=close,thigh=thigh,tlow=tlow,fthigh=fhigh,ftlow=flow,mkc=a,PE=macap,EP=eps,PB=b
                            ,Bv=bv,sector=sector,industry = industry,employ=Emp,city=city,country=country,phone=phone,web=website,title0 = title0,link0 = link0,
